...
{
    AVOutputFormat *oTsFmt = NULL;
    //Input AVFormatContext and Output AVFormatContext
    AVFormatContext *ifmt_ctx_v = NULL, *ifmt_ctx_a = NULL;
    AVFormatContext *rtp_mpegts_ofmt_ctx = NULL;
    ...
    const char *out_filename = "rtp://127.0.0.1:4320";//Output file URL
    ...
    //Output
    avformat_alloc_output_context2(&rtp_mpegts_ofmt_ctx, NULL, "rtp_mpegts", out_filename);
    if (!rtp_mpegts_ofmt_ctx) {
        printf("Could not create output context\n");
        ret = AVERROR_UNKNOWN;
        goto end;
    }
    oTsFmt = rtp_mpegts_ofmt_ctx->oformat;
    ...
    for (i = 0; i < ifmt_ctx_v->nb_streams; i++) {
        ...
        if (ifmt_ctx_v->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            AVStream *out_stream = avformat_new_stream(rtp_mpegts_ofmt_ctx, NULL);
            if (!out_stream) {
    ...
    av_dump_format(rtp_mpegts_ofmt_ctx, 0, out_filename, 1);
    printf("======================================\n");
    //Open output file
    if (!(oTsFmt->flags & AVFMT_NOFILE)) {
        if (avio_open(&rtp_mpegts_ofmt_ctx->pb, out_filename, AVIO_FLAG_WRITE) < 0) {
    ...
    //Write file header
    int erno = 0;
    if ((erno = avformat_write_header(rtp_mpegts_ofmt_ctx, NULL)) < 0) {
        char errstr[512] = { 0 };
    ...
    while (1) {
        ...
        if (av_compare_ts(cur_pts_v, ifmt_ctx_v->streams[videoindex_v]->time_base,
            ...
            if (av_read_frame(ifmt_ctx, &pkt) >= 0) {
                do {
                    ...
                    out_stream = rtp_mpegts_ofmt_ctx->streams[stream_index];
        ...
        //Write
        if (av_interleaved_write_frame(rtp_mpegts_ofmt_ctx, &pkt) < 0) {
    ...
    //Write file trailer
    av_write_trailer(rtp_mpegts_ofmt_ctx);
    ...
    /* close output */
    if (rtp_mpegts_ofmt_ctx && !(oTsFmt->flags & AVFMT_NOFILE))
        avio_close(rtp_mpegts_ofmt_ctx->pb);
    avformat_free_context(rtp_mpegts_ofmt_ctx);
